...
<script type="text/javascript">
	...
	function create_values(response){
        ...
        document.getElementById('pulse_data').innerHTML = parseFloat(response.meanBPM).toFixed(2) +' '+'bpm';
	...
	function setCurrentTraining(trainingType){
	...
	function pulseGauge(){
		...
		$('#gaugeContainer').jqxGauge('value', getPulseMean());	
	}
	
	function getPulseMean(){
		var mean = 0;
		for(var i = 0 ; i < window.pulse_data.length ; i++){
			mean += window.pulse_data[i];
		}
		mean = mean / window.pulse_data.length;
		return mean;
	}
